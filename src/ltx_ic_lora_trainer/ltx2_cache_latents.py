...
import ltx_ic_lora_trainer.cache_latents as cache_latents
from ltx_ic_lora_trainer.audio_io_utils import coerce_decoded_audio_to_channels_first
...
from ltx_ic_lora_trainer.dataset.image_video_dataset import (
    ARCHITECTURE_LTX2,
    AUDIO_EXTENSIONS,
    BaseDataset,
    IMAGE_EXTENSIONS,
...
from ltx_ic_lora_trainer.ltx_2.env import get_ltx2_env
from ltx_ic_lora_trainer.utils.model_utils import str_to_dtype
from ltx_ic_lora_trainer.utils.safetensors_utils import MemoryEfficientSafeOpen
    ...
    *,
    ...
    if not audio_only and isinstance(frame_count, int) and frame_count > 0 and waveform.shape[-1] > 0:
        ...
        if actual_duration > 0 and abs(actual_duration - expected_duration) / actual_duration > 0.01:
            from ltx_ic_lora_trainer.audio_utils import time_stretch_preserve_pitch
...
def _precache_sample_latents(args: argparse.Namespace, device: torch.device) -> None:
    ...
    from ltx_ic_lora_trainer.base_trainer import load_prompts
    from PIL import Image
...
def main() -> None:
    ...
    if not audio_only:
        ...
        from ltx_ic_lora_trainer.ltx_2.model.video_vae.tiling import TilingConfig, SpatialTilingConfig, TemporalTilingConfig
