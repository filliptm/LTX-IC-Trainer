...
from ltx_ic_lora_trainer.utils.device_utils import synchronize_device

logger = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO)


from ltx_ic_lora_trainer.modules.fp8_optimization_utils import load_safetensors_with_fp8_optimization
from ltx_ic_lora_trainer.utils.safetensors_utils import (
    ...
    if lora_weights_list is None or len(lora_weights_list) == 0:
        ...
        def weight_hook_func(model_weight_key, model_weight: torch.Tensor, keep_on_calc_device=False):
            ...
            for lora_weight_keys, lora_sd, multiplier, net_type in zip(
                ...
                if net_type == "loha":
                    from ltx_ic_lora_trainer.networks.loha import merge_weights_to_tensor as loha_merge

                    model_weight = loha_merge(model_weight, lora_name, lora_sd, lora_weight_keys, multiplier, calc_device)
                elif net_type == "lokr":
                    from ltx_ic_lora_trainer.networks.lokr import merge_weights_to_tensor as lokr_merge
