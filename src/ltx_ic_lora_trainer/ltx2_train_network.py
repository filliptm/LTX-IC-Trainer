...
from ltx_ic_lora_trainer.base_trainer import (
    NetworkTrainer,
)
from ltx_ic_lora_trainer.audio_supervision import (
...
from ltx_ic_lora_trainer.utils import model_utils
from ltx_ic_lora_trainer.ltx_2.model.transformer.fp8_device_utils import ensure_fp8_modules_on_device
from ltx_ic_lora_trainer.modules.nf4_optimization_utils import (
    is_nf4_module,
    DEFAULT_NF4_BLOCK_SIZE,
)
from ltx_ic_lora_trainer.ltx_2.env import apply_ltx2_tweaks
from ltx_ic_lora_trainer.ltx2_text_conditioning import (
    select_audio_text_embeds_for_audio_mode,
    select_video_text_embeds_for_video_mode,
    select_video_text_embeds_for_av_no_audio,
)
from ltx_ic_lora_trainer.ltx2_lycoris_runtime import (
...
from ltx_ic_lora_trainer.ltx2_sampling import LTX2SamplingMixin
from ltx_ic_lora_trainer.ltx2_model_loading import (  # noqa: F401 — re-exported for external callers
    detect_ltx2_dtype,
    detect_ltx2_config,
...
def ltx2_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Lazy re-export to avoid a circular import with ltx_ic_lora_trainer.ltx2_args."""
    from ltx_ic_lora_trainer.ltx2_args import ltx2_setup_parser as _ltx2_setup_parser
...
def main() -> None:
    """Lazy re-export to avoid a circular import with ltx_ic_lora_trainer.ltx2_args."""
    from ltx_ic_lora_trainer.ltx2_args import main as _main
...
class LTX2NetworkTrainer(LTX2SamplingMixin, NetworkTrainer):
    ...
    def _build_audio_ref_transformer_overrides(
        ...
        *,
        ...
        if ref_tokens > 0:
            from ltx_ic_lora_trainer.ltx_2.types import AudioLatentShape
    ...
    def _setup_audio_metrics(self, args: argparse.Namespace) -> None:
        ...
        if not getattr(args, "audio_metrics", False):
            return
        from ltx_ic_lora_trainer.audio_metrics import AudioMetricsConfig, AudioMetricsModule, parse_audio_metrics_args, _config_from_kwargs
    ...
    def _setup_hfato(self, args: argparse.Namespace) -> None:
        ...
        from ltx_ic_lora_trainer.hfato import HFATOConfig, parse_hfato_args
    ...
    def _setup_tarp_dcr(self, args: argparse.Namespace) -> None:
        ...
        from ltx_ic_lora_trainer.preservation import parse_preservation_args
    ...
    def _setup_preservation(self, args: argparse.Namespace, accelerator: Accelerator) -> None:
        ...
        from ltx_ic_lora_trainer.preservation import PreservationConfig, PreservationHelper, parse_preservation_args
    ...
    def _setup_crepa(self, args: argparse.Namespace, accelerator: Accelerator,
        ...
        from ltx_ic_lora_trainer.crepa import CREPAConfig, CREPAModule, parse_crepa_args
    ...
    def _setup_self_flow(
        ...
        from ltx_ic_lora_trainer.self_flow import (
            SelfFlowConfig,
            SelfFlowModule,
    ...
    def _apply_network_initialization(self, args: argparse.Namespace, network=None) -> None:
        ...
        if hasattr(args, "_network_init_params"):
            ...
            if "lokr_norm" in init_params:
                ...
                try:
                    from ltx_ic_lora_trainer.networks.lycoris_extensions import init_lokr_network_with_perturbed_normal
    ...
    def _load_ltx2_checkpoint_config(self, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        from ltx_ic_lora_trainer.ltx_2.loader.sft_loader import SafetensorsModelStateDictLoader
    ...
    def architecture(self) -> str:
        ...
        from ltx_ic_lora_trainer.dataset.image_video_dataset import ARCHITECTURE_LTX2
    ...
    def architecture_full_name(self) -> str:
        ...
        from ltx_ic_lora_trainer.dataset.image_video_dataset import ARCHITECTURE_LTX2_FULL
    ...
    def post_save_checkpoint_hook(self, args, ckpt_file, ckpt_name, accelerator, force_sync_upload=False):
        ...
        try:
            from ltx_ic_lora_trainer.ltx_2.convert_lora_to_comfy import convert_lora_to_comfy
            ...
            if args.huggingface_repo_id is not None:
                from ltx_ic_lora_trainer.utils import huggingface_utils
    ...
    def load_transformer(
        ...
        if self._train_connectors:
            from ltx_ic_lora_trainer.networks.lora_ltx2 import load_connectors_from_checkpoint
            from ltx_ic_lora_trainer.ltx_2.loader.sft_loader import SafetensorsModelStateDictLoader
    ...
    def _load_vae_impl(self, args: argparse.Namespace, vae_dtype: torch.dtype, vae_path: str):
        ...
        from ltx_ic_lora_trainer.ltx_2.loader.single_gpu_model_builder import SingleGPUModelBuilder
        from ltx_ic_lora_trainer.ltx_2.model.video_vae import VideoDecoderConfigurator, VAE_DECODER_COMFY_KEYS_FILTER
    ...
    def call_dit(
        ...
        if ref_latents is not None:
            from ltx_ic_lora_trainer.ltx_2.components.patchifiers import VideoLatentPatchifier, get_pixel_coords
            from ltx_ic_lora_trainer.ltx_2.guidance.perturbations import BatchedPerturbationConfig
            from ltx_ic_lora_trainer.ltx_2.model.transformer.modality import Modality
            from ltx_ic_lora_trainer.ltx_2.types import SpatioTemporalScaleFactors, VideoLatentShape
        ...
        if ic_lora_strategy == "av_ic" and ref_latents is not None:
            ...
            from ltx_ic_lora_trainer.ltx_2.guidance.perturbations import BatchedPerturbationConfig
            from ltx_ic_lora_trainer.ltx_2.model.transformer.modality import Modality
            from ltx_ic_lora_trainer.ltx_2.types import AudioLatentShape, SpatioTemporalScaleFactors, VideoLatentShape
            from ltx_ic_lora_trainer.networks.lora_ltx2 import _split_av_context
