...
from ltx_ic_lora_trainer.base_trainer import read_config_from_file, setup_parser_common
from ltx_ic_lora_trainer.ltx_2.env import apply_ltx2_tweaks
from ltx_ic_lora_trainer.ltx2_lycoris_runtime import apply_lycoris_preset_before_network_creation, is_lycoris_requested, process_lycoris_config
from ltx_ic_lora_trainer.ltx2_train_network import IC_LORA_STRATEGIES
...
def main() -> None:
    ...
    # Allow --vae override for LTX-2 to avoid reloading VAE from the full 46GB
    # checkpoint during sampling (which can trigger VRAM-fragmentation crashes).
    # If --vae is not provided, default to --ltx2_checkpoint (VAE is baked in).
    if getattr(args, "vae", None) is None:
    ...
    from ltx_ic_lora_trainer.ltx2_train_network import LTX2NetworkTrainer
