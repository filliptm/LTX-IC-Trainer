...
class DatasetEntry(BaseModel):
    name: str = ""  # Display name for the UI; empty = legacy dataset
    thumbnail: str = ""  # Path to a thumbnail image (optional)
    type: Literal["video", "image", "audio"] = "video"
    directory: str = ""
    cache_directory: str = ""
    # IC-LoRA v2v reference: the raw reference (conditioning) directory and
    # the cache directory its encoded latents are written to. Both are read
    # by ltx2_cache_latents.encode_and_save_reference_latents and consumed
    # by VideoDataset / ImageDataset during training when
    # --ic_lora_strategy is v2v (or auto-detected from an IC-LoRA preset).
