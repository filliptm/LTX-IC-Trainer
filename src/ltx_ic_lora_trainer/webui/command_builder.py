...
from ltx_ic_lora_trainer.webui.toml_export import (
    _write_slider_toml,
...
def _find_script(name: str) -> str:
    """Find a script in the ltx_ic_lora_trainer package."""
    import ltx_ic_lora_trainer
    pkg_dir = Path(ltx_ic_lora_trainer.__file__).parent
    script = pkg_dir / name
    if script.exists():
        return str(script)
...
def build_training_cmd(config: ProjectConfig) -> list[str]:
    ...
    # The webui always forces --save_state_on_train_end so that Stop
    # produces a recoverable state for the Resume flow, regardless of
    # what the project config says.
    if "--save_state_on_train_end" not in cmd:
