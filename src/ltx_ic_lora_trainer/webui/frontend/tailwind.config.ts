...
export default {
  ...
  plugins: [
    // Radix data-state animation utilities (animate-in, fade-in-0, etc.)
    function ({ addUtilities }: { addUtilities: (utils: Record<string, Record<string, string>>) => void }) {
      addUtilities({
        ".animate-in": {
          "animation-name": "fade-in, zoom-in, slide-in-from-top, slide-in-from-bottom, slide-in-from-left, slide-in-from-right",
