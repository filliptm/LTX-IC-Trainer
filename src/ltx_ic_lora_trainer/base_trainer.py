...
from ltx_ic_lora_trainer import convert_lora
from ltx_ic_lora_trainer.dataset import config_utils
from ltx_ic_lora_trainer.dataset.audio_quota_sampler import (
    build_audio_sampler,
    split_concat_indices_by_audio,
    sync_dataset_group_epoch_without_loading,
)
from ltx_ic_lora_trainer.modality_freezer import ModalityFreezer
from ltx_ic_lora_trainer.cross_task_synergy import compute_cross_task_synergy_losses
from ltx_ic_lora_trainer.audio_loss_balance import (
...
from ltx_ic_lora_trainer.ogm_ge import compute_ogm_ge_coefficients, maybe_add_ogm_ge_gradient_noise
from ltx_ic_lora_trainer.modules.group_lr_scheduler import GroupWarmupScheduler, parse_group_lr_warmup_args
from ltx_ic_lora_trainer.modules.lr_schedulers import RexLR
from ltx_ic_lora_trainer.modules.scheduling_flow_match_discrete import FlowMatchDiscreteScheduler
import ltx_ic_lora_trainer.networks.lora as lora_module
from ltx_ic_lora_trainer.networks.optimizer_params_compat import prepare_optimizer_params_compat
from ltx_ic_lora_trainer.dataset.config_utils import BlueprintGenerator, ConfigSanitizer

import logging

from ltx_ic_lora_trainer.utils import huggingface_utils, model_utils, train_utils, sai_model_spec
...
class NetworkTrainer:
    ...
    def get_optimizer(self, args, trainable_params: list[torch.nn.Parameter]) -> tuple[str, str, torch.optim.Optimizer]:
        ...
        if optimizer_type.endswith("8bit".lower()):
            ...
            from ltx_ic_lora_trainer.optimizers.automagic import Automagic
    ...
    def train(self, args):
        ...
        # webui metrics writer (lazy import, no-op when --gui is not set)
        gui_metrics = None
        if getattr(args, "gui", False) and accelerator.is_main_process:
            from ltx_ic_lora_trainer.webui.metrics_writer import create_metrics_writer
        ...
        from ltx_ic_lora_trainer.ltx2_train_network import load_ltx2_model
                ...
                for val_step, batch in enumerate(val_iter):
                    ...
                    if _hfato_config is not None and latents_tensor.dim() == 5:
                        ...
                        if _hfato_rand.random() < _hfato_config.probability:
                            from ltx_ic_lora_trainer.hfato import degrade_latents
                    ...
                    if dict_output:
                        ...
                        if _hfato_data is not None:
                            from ltx_ic_lora_trainer.hfato import hfato_x0_loss
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if accelerator.sync_gradients:
                    ...
                    if should_sampling or should_saving:
                        set_trainer_eval_mode()
                        # Save checkpoint and state BEFORE sampling so a crash during sampling
                        # doesn't lose the checkpoint.
                        if should_saving:
