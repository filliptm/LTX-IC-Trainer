...
| POST | `/api/project` | Create a new project under `<repo_root>/projects/<slug>/`. Body: `{name}`. Any incoming `project_dir` is ignored — the location is app-managed. If a project with that slug already exists, a numeric suffix is appended (`_2`, `_3`, …). The project is seeded with a single default `DatasetEntry` (same name as the project, `type=video`) and its `datasets/<slug>/{target,reference,cache,cache_ref}/` directory tree, so the user lands directly on a usable workspace without an explicit dataset-create step. App-managed output paths (`training.output_dir = <project_dir>/output`, `training.logging_dir = <project_dir>/logs`, `inference.output_dir = <project_dir>/inference`) are also set automatically. |
| PUT | `/api/project` | Update the currently loaded project. Re-applies app-managed paths (`output_dir`, `logging_dir`, `inference.output_dir`) on every save so the user can't override them via direct API or a stale frontend cache. |
| DELETE | `/api/project` | Close the project (clears app state) |
| POST | `/api/project/load` | Load a project from the app-managed projects directory. Body: `{slug}` *or* `{path}`. Paths outside `<repo_root>/projects/` are rejected with 400. Heals empty/stale `output_dir`/`logging_dir`/`inference.output_dir` on the loaded config and persists them to disk. |
...
| GET | `/api/dataset/{index}/buckets` | Preview which LTX-2 aspect-ratio buckets the dataset's files will land in **and** the source-clip duration distribution **and** an estimated training-sample count. Query: `width` / `height` (default = entry's `resolution_w/h`), `slot` ("target"\|"reference"), `max_files` (default 2000), plus the trainer-effective params `target_frames` / `frame_extraction` / `frame_stride` / `frame_sample` / `target_fps` / `max_frames` (default to the entry's saved values). Reproduces `BucketSelector`'s 32-px-grid bucket math without importing dataset code, walks the directory non-recursively, probes per-file dimensions + frame count + fps via PIL (images) / cv2 (videos) header reads — no full decode, single capture handle per video. Returns `{target_area, target_resolution, buckets: [{resolution, aspect, count, items}], unreadable, scanned, truncated, duration_distribution: {bins, count, total_s, avg_s, p50, p95, min_s, max_s}, video_count, image_count, estimated_training_samples}`. The estimator simulates the loader's chunk/slide/head/uniform/full math against the trainer params; images count as 1 sample each. Audio + unknown extensions are skipped; corrupt headers go to `unreadable`. |
...
| `DatasetSettings` | Fixed right panel (400px) with dataset-level settings: type, target width/height (with tooltips explaining bucketing), batch size, repeats, caption extension, video-only options. Includes an "Aspect-aware bucketing on" badge above the resolution fields plus live `BucketPreview` and `DurationPreview` panels below them — when the user types a new target W/H or video-options value, both panels re-run (300ms debounce) and share a single network request via TanStack's structural cache key. The user can see which buckets their dataset will populate, the source-clip duration distribution, and the estimated training-sample count before saving. Own `FormProvider` instance; saves via `PUT /api/project`. |
| `BucketPreview` | Live histogram of LTX-2 bucket assignments, rendered inside `DatasetSettings`. Uses `useDatasetBuckets` to call `GET /api/dataset/{index}/buckets`. Each row shows a horizontal bar (% of max count), the bucket resolution, an aspect-ratio label (e.g. `9:16 portrait`, `16:9 landscape`, `square`), and the count. Click a row to expand the file list. Surfaces `unreadable` (corrupt-header files) in a collapsible warning row and `truncated` when the dataset exceeds `max_files=2000`. Accepts an optional `trainer` prop so its query key matches `DurationPreview`'s — TanStack Query then dedupes the two components into a single network request. |
| `DurationPreview` | Source-clip duration histogram + trainer-effective training-sample count, rendered below `BucketPreview` inside `DatasetSettings`. Reads from the same `useDatasetBuckets` hook (cv2 returns dimensions + frame count + fps in one capture, so duration data is free). Shows: video/image counts, summary line (avg / median / p95 / total), 5-bin histogram (0–2s, 2–5s, 5–10s, 10–30s, 30s+), and a highlighted "With current settings: N training samples per epoch" panel that updates live as the user edits the video options. Image-only datasets render the sample-count panel without a histogram. |
